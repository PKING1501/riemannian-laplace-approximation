...
def main(args):
    ...
    for cls, ax in enumerate(axes):
        ...
        ax.axis("off")
    plt.suptitle("Example Images from Each Class", y=1.05)
    save_plot(fig, "class_examples")
    
    # 2. Random training samples
    rand_indices = torch.randint(0, len(train_dataset), (15,))
    ...
    sns.kdeplot(P_test_MAP.max(1)[0].cpu().numpy(), label='MAP', ax=ax)
    sns.kdeplot(P_test_LAPLACE.max(1)[0].cpu().numpy(), label='Laplace', ax=ax)
    sns.kdeplot(P_test_OURS.max(1)[0].cpu().numpy(), label='Riemannian', ax=ax)
    ...
    # 2. Calibration Plots
    def plot_calibration(y_true, probs, title, filename):
        fig, ax = plt.subplots(figsize=(8, 6))
        bins = np.linspace(0, 1, 11)
        bin_centers = (bins[:-1] + bins[1:]) / 2
        
        confidences = probs.max(axis=1)[0].cpu().numpy()
        predictions = probs.argmax(axis=1).cpu().numpy()
        accuracies = (predictions == y_true).astype(float)
        
        bin_indices = np.digitize(confidences, bins) - 1
        bin_acc = np.array([accuracies[bin_indices == i].mean() 
                          for i in range(len(bins)-1)])
        bin_conf = np.array([confidences[bin_indices == i].mean() 
                           for i in range(len(bins)-1)])
        
        ax.plot([0, 1], [0, 1], 'k--', label='Perfectly calibrated')
        ax.plot(bin_conf, bin_acc, 's-', label='Model')
    ...
    plot_calibration(y_true, P_test_MAP, "MAP Calibration", "map_calibration")
    plot_calibration(y_true, P_test_LAPLACE, "Laplace Calibration", "laplace_calibration")
    plot_calibration(y_true, P_test_OURS, "Riemannian Calibration", "riemannian_calibration")

    # 3. t-SNE of features with confidence
    ...
    scatter = ax.scatter(features_2d[:, 0], features_2d[:, 1], c=labels, cmap='tab10', alpha=0.6)
    plt.colorbar(scatter, label='Digit Class')
    ax.set_title("t-SNE Visualization by Class")
    ax.grid(True)
    save_plot(fig, "tsne_by_class")
    
    # t-SNE by confidence
    fig, ax = plt.subplots(figsize=(10, 8))
    scatter = ax.scatter(features_2d[:, 0], features_2d[:, 1], c=confidences, cmap='viridis', alpha=0.6)
    plt.colorbar(scatter, label='Confidence')
    ax.set_title("t-SNE Visualization by Confidence")
    ax.grid(True)
    save_plot(fig, "tsne_by_confidence")

    # 4. Error Analysis
    def plot_error_analysis(y_true, y_pred, probs, title, filename):
        ...
        sns.kdeplot(probs.max(1)[0].cpu().numpy()[correct], 
                   label='Correct', ax=ax1)
        sns.kdeplot(probs.max(1)[0].cpu().numpy()[incorrect], 
                   label='Incorrect', ax=ax1)
    ...
    plot_error_analysis(y_true, P_test_MAP.argmax(1).cpu().numpy(), 
                       P_test_MAP, "MAP Error Analysis", "map_error_analysis")
    plot_error_analysis(y_true, P_test_LA_pred.cpu().numpy(), 
                       P_test_LAPLACE, "Laplace Error Analysis", "laplace_error_analysis")
    plot_error_analysis(y_true, P_test_OURS_pred.cpu().numpy(), 
                       P_test_OURS, "Riemannian Error Analysis", "riemannian_error_analysis")

    # 5. Metric Comparison Bar Plot
    metrics = ['Accuracy', 'NLL', 'ECE']
    map_vals = [dict_MAP[m] for m in metrics]
    la_vals = [dict_LA[m] for m in metrics]
    our_vals = [dict_OUR[m] for m in metrics]
    ...
    ax.bar(x, la_vals, width, label='Laplace')
    ax.bar(x + width, our_vals, width, label='Riemannian')
    ...
    ax.legend()
    save_plot(fig, "metric_comparison")
