...
# Custom complex intertwined dataset (radial swirl pattern)
def generate_complex_data(n_samples=1000, noise=0.2):
    n = n_samples // 2
    r = np.linspace(0.5, 1.5, n)
    theta1 = np.linspace(0, 4 * np.pi, n)
    theta2 = np.linspace(0.5 * np.pi, 4.5 * np.pi, n)

    # Class 0
    x1 = r * np.cos(theta1) + noise * np.random.randn(n)
    y1 = r * np.sin(theta1) + noise * np.random.randn(n)
    class0 = np.vstack((x1, y1)).T
    labels0 = np.zeros(n, dtype=int)

    # Class 1
    x2 = r * np.cos(theta2) + noise * np.random.randn(n)
    y2 = r * np.sin(theta2) + noise * np.random.randn(n)
    class1 = np.vstack((x2, y2)).T
    labels1 = np.ones(n, dtype=int)
...
def main(args):
    ...
    X, y = generate_complex_data(n_samples=1000, noise=0.25)

    # Visualize
    plt.figure(figsize=(6, 6))
    plt.scatter(X[y == 0, 0], X[y == 0, 1], c="orange", edgecolors="black", s=45, label="Class 0")
    plt.scatter(X[y == 1, 0], X[y == 1, 1], c="violet", edgecolors="black", s=45, label="Class 1")
    plt.legend()
    plt.xticks([], [])
    plt.yticks([], [])
    plt.title("Complex Nonlinear Synthetic Dataset")
    plt.savefig("synthetic_complex_plot.png")
    ...
    x_valid, x_test, y_valid, y_test = sklearn.model_selection.train_test_split(
        x_temp, y_temp, train_size=0.5, random_state=seed, shuffle=True
    )

    # Optional: Limit number of training samples
    x_train = x_train[:265]
    y_train = y_train[:265]
