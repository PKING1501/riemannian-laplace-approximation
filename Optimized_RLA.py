...
# Assuming the geometry functions (expmap) and other required imports are defined

def compute_weights(n, V_LA, manifold, ll_map, map_solution, feature_extractor_map, model, model2, x_train, y_train, f_MAP, la, args, subset_of_weights):
    v = V_LA[n, :].reshape(-1, 1)

    # Handle different weight subsets
    if subset_of_weights == "last_layer":
        ...
        # Subsampling for different batches if enabled
        ...
        return torch.from_numpy(_new_weights.reshape(-1))
...
def main(args):
    ...
    print(f"Time taken for computing manofold: {end1 - start1:.4f} seconds")
    start2 = time.perf_counter()
    # now i have my manifold and so I can solve the expmap
    NUM_CORES = 8  # Number of parallel workers

    # Parallelized computation function
    results = Parallel(n_jobs=NUM_CORES)(
        delayed(compute_weights)(
            n, V_LA, manifold, ll_map, map_solution, feature_extractor_map, model, model2, x_train, y_train, f_MAP, la, args,subset_of_weights
        ) 
        for n in range(n_posterior_samples)
    )

    # Stack results into a single tensor
    weights_ours = torch.stack(results)

    # Ensure that the size and type of weights_ours are correct
    print(weights_ours.shape)
    end2 = time.perf_counter()
    print(f"Time taken for computing weights using parallelization: {end2 - start2:.4f} seconds")
    ...
    print(f"Time taken for computing weights using LA: {end3 - start3:.4f} seconds")
